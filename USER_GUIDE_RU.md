...
В аддоне четыре основные панели (доступны во вкладке **Difference Machine** в боковой панели `N`):
...
### 4. Mesh Diff (Анализ различий мешей)
- Анализ различий между версиями мешей
- Визуализация изменений с помощью vertex colors
...
**Структура аддона:**
- `empty_files/empty.blend` - шаблон пустого файла Blender для фонового экспорта мешей
  - Используется при создании mesh-only коммитов
  - Обеспечивает стабильность экспорта без влияния на текущий проект
...
- **Фоновый экспорт**: Меши экспортируются в фоновом процессе Blender
  - Текущая сцена проекта не изменяется во время создания коммита
  - Используется шаблон `empty.blend` для создания файлов коммитов
  - Обеспечивает стабильность и предотвращает потерю несохраненных изменений
...
5. Diff автоматически вычисляется и сохраняется
...
### Анализ различий мешей (Mesh Diff)

**Как использовать:**
1. Выберите меш в сцене
2. Откройте панель **"Mesh Diff"** (4-я панель во вкладке Difference Machine)
3. Если diff еще не вычислен:
   - Выберите коммит из списка (показываются последние 5 коммитов)
   - Нажмите **"Compare"** рядом с коммитом
   - Diff будет вычислен между текущим мешем и версией из коммита
4. Просмотрите статистику:
   - **Изменения геометрии**: Количество добавленных/удаленных/измененных вершин и граней
   - **Процент изменений**: Насколько изменилась геометрия (0-100%)
   - **Метрики смещения**: Максимальное и среднее смещение вершин
   - **Изменения материалов**: Изменения текстур и нодов (если есть)

**Визуализация:**
1. После вычисления diff выберите схему цветов:
   - **Displacement**: Цвет по величине смещения вершин (красный = больше изменений, зеленый = меньше)
   - **Added**: Зеленый для добавленных вершин
   - **Removed**: Красный для удаленных вершин
   - **Modified**: Желтый для измененных вершин
2. Нажмите **"Apply Visualization"**
   - Viewport переключается в режим отображения vertex colors
   - Материалы временно скрываются для показа vertex colors
   - Изменения подсвечиваются согласно выбранной схеме

**Важные замечания:**
- **Материалы**: Когда активна визуализация vertex colors, материалы скрыты. Чтобы снова увидеть материалы:
  - Нажмите **"Clear Diff"** для удаления визуализации, или
  - Вручную переключите режим цветов viewport: Нажмите `N` → Viewport Shading → Color → `Material`
- **Данные diff**: Статистика diff сохраняется в свойствах сцены и остается до очистки
- **Интеграция с сравнением**: При использовании кнопки "Compare" в панели Load Commit, diff автоматически вычисляется

**Очистка diff:**
- Нажмите кнопку **"Clear Diff"** для удаления:
  - Статистики diff
  - Визуализации vertex colors
  - Восстановления отображения материалов

### Версионирование текстур (Texture Versioning)

**Как это работает:**
- Текстуры автоматически версионируются при создании коммитов.
- Каждая текстура хранится независимо со своим уникальным hash.
- Неизмененные текстуры автоматически дедуплицируются - один и тот же файл текстуры хранится только один раз.
- Одна текстура может использоваться многими мешами без дублирования.
- История текстур отслеживается отдельно от коммитов мешей.

**Преимущества:**
- Эффективное хранение: неизмененные текстуры не дублируются.
- Независимое версионирование: изменения текстур не требуют новых коммитов мешей.
- Простое отслеживание: видно, какие текстуры используются в каких коммитах.
...
4. **Создавайте mesh-only коммиты** для быстрых сохранений геометрии
   - Фоновый экспорт не затрагивает текущую сцену - можно продолжать работу
   - Не нужно сохранять проект перед созданием mesh-only коммита
...
- **Версионирование текстур**: Текстуры версионируются независимо от мешей
  - Одна текстура может использоваться многими мешами без дублирования
  - История текстур отслеживается отдельно
  - Эффективное хранение с content-addressable hashing
...
**Q: Изменяется ли текущая сцена при создании mesh-only коммита?**  
A: Нет. Меши экспортируются в фоновом процессе Blender с использованием шаблона `empty.blend`. Ваша текущая сцена не изменяется, и вы можете продолжать работу во время создания коммита.
