...
class DF_PT_diff_panel(Panel):
    """Panel for displaying mesh diff statistics."""
    bl_label = "Mesh Diff"
    bl_idname = "DF_PT_diff_panel"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_category = "Difference Machine"
    bl_order = 4
    ...
    @classmethod
    def poll(cls, context):
        """Show panel only if repository is initialized and mesh is selected."""
        from ..operators.operator_helpers import is_repository_initialized
        if not is_repository_initialized(context):
            return False
        
        active_obj = context.active_object
        return active_obj and active_obj.type == 'MESH'
    ...
    def draw(self, context):
        ...
        # Check if diff is computed
        if 'df_diff_result' not in scene:
            box = layout.box()
            box.label(text="No diff computed", icon='INFO')
            box.label(text="Select a commit and compute diff")
            ...
            if hasattr(scene, 'df_commits') and scene.df_commits:
                ...
                for i, commit in enumerate(commits[:5]):  # Show last 5 commits
                    row = box.row()
                    row.label(text=f"{commit.message[:30]}...")
                    op = row.operator("df.compute_mesh_diff", text="Compare", icon='ZOOM_ALL')
                    op.commit_hash = commit.hash
            return
        
        # Display diff statistics
        diff_dict = scene['df_diff_result']
        stats = diff_dict.get('statistics', {})
        
        box = layout.box()
        box.label(text="Diff Statistics", icon='INFO')
        
        # Geometry statistics
        col = box.column(align=True)
        col.label(text="Geometry Changes:")
        row = col.row(align=True)
        row.label(text=f"Vertices:")
        row.label(text=f"+{stats.get('vertices_added_count', 0)} "
                      f"-{stats.get('vertices_removed_count', 0)} "
                      f"~{stats.get('vertices_modified_count', 0)}")
        
        row = col.row(align=True)
        row.label(text=f"Faces:")
        row.label(text=f"+{stats.get('faces_added_count', 0)} "
                      f"-{stats.get('faces_removed_count', 0)}")
        
        col.separator()
        row = col.row(align=True)
        row.label(text=f"Geometry change:")
        row.label(text=f"{stats.get('geometry_change_percent', 0.0):.1f}%")
        
        if stats.get('max_vertex_displacement', 0) > 0:
            row = col.row(align=True)
            row.label(text=f"Max displacement:")
            row.label(text=f"{stats.get('max_vertex_displacement', 0.0):.4f}")
            
            row = col.row(align=True)
            row.label(text=f"Avg displacement:")
            row.label(text=f"{stats.get('avg_vertex_displacement', 0.0):.4f}")
        
        # Material statistics
        if (stats.get('textures_added_count', 0) > 0 or
            stats.get('textures_removed_count', 0) > 0 or
            stats.get('textures_modified_count', 0) > 0 or
            stats.get('nodes_added_count', 0) > 0 or
            stats.get('nodes_removed_count', 0) > 0 or
            stats.get('nodes_modified_count', 0) > 0):
            
            col.separator()
            col.label(text="Material Changes:")
            row = col.row(align=True)
            row.label(text=f"Textures:")
            row.label(text=f"+{stats.get('textures_added_count', 0)} "
                          f"-{stats.get('textures_removed_count', 0)} "
                          f"~{stats.get('textures_modified_count', 0)}")
            
            row = col.row(align=True)
            row.label(text=f"Nodes:")
            row.label(text=f"+{stats.get('nodes_added_count', 0)} "
                          f"-{stats.get('nodes_removed_count', 0)} "
                          f"~{stats.get('nodes_modified_count', 0)}")
            
            row = col.row(align=True)
            row.label(text=f"Material change:")
            row.label(text=f"{stats.get('material_change_percent', 0.0):.1f}%")
        
        # Visualization controls
        layout.separator()
        box = layout.box()
        box.label(text="Visualization", icon='RESTRICT_VIEW_OFF')
        
        row = box.row()
        row.prop(context.scene, 'df_diff_color_scheme', expand=True)
        
        row = box.row()
        op = row.operator("df.visualize_mesh_diff", text="Apply Visualization", icon='MATERIAL')
        op.color_scheme = getattr(context.scene, 'df_diff_color_scheme', 'displacement')
        
        # Clear diff button
        layout.separator()
        row = layout.row()
        op = row.operator("df.clear_mesh_diff", text="Clear Diff", icon='X')
